...
export default async function handler(
  ...
  try {
    if (!block) {
      //This calls to openAI with our points of interest prompt, and returns an object with param: choices.
      const chatGPTFetch = await fetch(
      ...
      let pointsOfInterest = await chatGPTFetch.json();
